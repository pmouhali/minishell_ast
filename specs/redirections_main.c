...
int		main(int ac, char **av, char **envp) // redirections simple
{
	...
	ast->left = btree_node_new(OPERATOR_ARG, oparg); // the file where the output is redirected
	ast->right = btree_node_new(COMMAND, args); // the command
	...
	oparg = (char**)ft_tabdup((const void**)sa_oparg, (void*)&ft_strdup); // alloue de nouveau le tableau pour btree_delete : sinon double free
...
int		main(int ac, char **av, char **envp) // redirection multiples
{
	int pc_r;
	environment = (char**)ft_tabdup((const void**)envp, (void*)&ft_strdup); // sert à simuler l'environnment (on en a besoin pour execve principalement)
	char *sa_oparg[] = {"__1.testfile", NULL}; char **oparg;
	char *sa_oparg2[] = {"__2.testfile", NULL}; char **oparg2;
	char **tf3;
	char **args;
	t_node *ast;
	...
	oparg = (char**)ft_tabdup((const void**)sa_oparg, (void*)&ft_strdup);
	oparg2 = (char**)ft_tabdup((const void**)sa_oparg2, (void*)&ft_strdup);

	ast = btree_node_new(REDIR_OUT_1, NULL);
	ast->left = btree_node_new(OPERATOR_ARG, oparg); // the file where the output is redirected
	ast->right = btree_node_new(REDIR_OUT_1, NULL); // the command
	ast->right->left = btree_node_new(OPERATOR_ARG, oparg2); // the file where the output is redirected
	...
	ast = btree_node_new(REDIR_OUT_2, NULL);
	ast->left = btree_node_new(OPERATOR_ARG, oparg2); // the file where the output is redirected
	ast->right = btree_node_new(REDIR_OUT_2, NULL); // the command
	ast->right->left = btree_node_new(OPERATOR_ARG, oparg); // the file where the output is redirected
	ast->right->right = btree_node_new(COMMAND, args); // the command
	...
	ast->right = btree_node_new(REDIR_OUT_2, NULL); // the command
	ast->right->left = btree_node_new(OPERATOR_ARG, oparg2); // the file where the output is redirected
	ast->right->right = btree_node_new(REDIR_OUT_1, NULL); // the command
	ast->right->right->left = btree_node_new(OPERATOR_ARG, tf3); // the command
	ast->right->right->right = btree_node_new(COMMAND, args); // the command
